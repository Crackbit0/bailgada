    ...
    def init_language_model(self, model_name: Optional[str] = None, temperature: Optional[float] = None):
        ...
        try:
            if self.provider == 'openai':
                ...
                print(f"--- ModelOrchestrator.init_language_model: ChatOpenAI for OpenRouter initialized with model: {model_to_use} ---")
            elif self.provider == 'deepseek':
                print(f"--- ModelOrchestrator.init_language_model: Initializing ChatOpenAI for {self.provider} ---")
                # Fallback to OpenAI
        ...
        try:
            ...
            if self.provider == 'openai':
                ...
                try:
                    ...
                    print(f"Successfully generated OpenRouter completion with {len(response_text) if response_text else 0} characters")
                except Exception as e:
            ...
            try:
                ...
                try:
                    ...
                    if first_brace != -1:
                        ...
                        print(f"DEBUG: Extracted JSON from position {first_brace} to {end_pos} ({len(potential_json)} chars)")
